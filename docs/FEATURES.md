...
| projects | Présent : création, états, archivage, import/export ; import non atomique. | `db.py:import_project`, `export_project`, `app.py` | `tests/test_database.py`, `tests/test_ui_smoke.py` |
...
| screenplay.editor | Partiel : blocs typés, IDs, raccourcis, complétion et aperçu. Texte/JSON concurrents ; versions textuelles. | `screenplay_model.py`, `app.py:ScreenplayEditor`, `_load_script_document`, `_save_script`, `_snapshot_script` | `tests/test_screenplay_model.py`, `tests/test_screenplay_persistence.py`, `tests/test_editor_polish.py`, `tests/test_layout_followup.py` |
...
Qt hors écran ne remplace pas un contrôle Fedora réel. Les interruptions d’import/sauvegarde, restauration structurée, gros projets et fidélité Unicode/FDX demandent des tests supplémentaires.
